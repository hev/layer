...
const CATALOG: &[MetricDoc] = &[
    ...
    MetricDoc {
        ...
        labels: &["namespace", "store_kind", "model", "serving"],
        description: "Embedding tokens echoed by the serving provider.",
        example_promql: "sum by (namespace, model, serving) (increase(hevlayer_embed_tokens_total[1h]))",
        alert: None,
    },
    MetricDoc {
        name: "hevlayer_embed_compute_seconds_total",
        kind: MetricKind::Counter,
        family: MetricFamily::Cost,
        labels: &["namespace", "store_kind", "model", "serving"],
        description: "Embedding latency echoed by the serving provider, in seconds.",
        example_promql: "sum by (namespace, model, serving) (increase(hevlayer_embed_compute_seconds_total[1h]))",
        alert: None,
    },
    MetricDoc {
